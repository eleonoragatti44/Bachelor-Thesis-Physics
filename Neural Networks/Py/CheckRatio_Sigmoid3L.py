...
y_train_def1 = f.normalization1(y_train_def1, data_max, data_min)
X_val_def = validation_def[:,:2]
y_val_def1 = validation_def[:,2]
y_val_def1 = f.normalization1(y_val_def1, data_max, data_min)
...
predicted_fwhmx = f.inverse_normalization1(predicted_fwhmx, data_max, data_min)
true_fwhmx = f.inverse_normalization1(true_fwhmx, data_max, data_min)

t = [np.array(X_train_def[:,0]), np.array(X_train_def[:,1])]
v = [np.array(X_val_def[:,0]), np.array(X_val_def[:,1])]
popt, pcov = curve_fit( f.func, t, f.inverse_normalization1(np.array(y_train_def1), data_max, data_min ))
function = f.func(v, *popt)

net_fwhmx = predicted_fwhmx.detach().numpy()
int_fwhmx = f.inverse_normalization1(np.array(y_val_def1), data_max, data_min)
...
y_train_def2 = f.normalization1(y_train_def2, data_max, data_min)
X_val_def = validation_def[:,:2]
y_val_def2 = validation_def[:,3]
y_val_def2 = f.normalization1(y_val_def2, data_max, data_min)

predicted_fwhmy = torch.squeeze( net(X_val_def).float() )
true_fwhmy = torch.squeeze( y_val_def2.float() )

# inverto la normalizzazione in modo da tornare al dominio di partenza
predicted_fwhmy = f.inverse_normalization1(predicted_fwhmy, data_max, data_min)
true_fwhmy = f.inverse_normalization1(true_fwhmy, data_max, data_min)

t1 = [np.array(X_train_def[:,0]), np.array(X_train_def[:,1])]
v1 = [np.array(X_val_def[:,0]), np.array(X_val_def[:,1])]
popt1, pcov1 = curve_fit( f.func, t1, f.inverse_normalization1(np.array(y_train_def2), data_max, data_min ))
function1 = f.func(v1, *popt1)

net_fwhmy = predicted_fwhmy.detach().numpy()
int_fwhmy = f.inverse_normalization1(np.array(y_val_def2), data_max, data_min)
...
np.savetxt(f'./ErrorPlot/Sigmoid_3L_CheckRatio', err)
