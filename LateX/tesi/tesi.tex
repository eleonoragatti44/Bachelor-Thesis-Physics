...
Le reti neurali per la regressione entrano in gioco quando, a partire da coppie input-output, si vuole determinare la funzione che approssimi al meglio la relazione. Quest'ultimo tipo di reti neurali è stato utilizzato per effettuare la stima delle proprietà di un diagramma di radiazione ~\ref{reti_neurali}.
...
\begin{figure}[!ht]
	...
	\caption{Schema di una rete neurale con tre hidden layers. Immagine presa da \textit{Deep Learning with PyTorch}, E. Stevens, L. Antiga\cite{stevens}.}
	\label{schema_rete}
\end{figure}
...
Un neurone è rappresentato da una funzione non lineare, detta \textbf{funzione di attivazione}, applicata ad una trasformazione lineare. Ogni neurone è caratterizzato da due parametri liberi: \textit{w (weight)} e \textit{b (bias)}. L'espressione matematica per un singolo neutrone è quindi
\[o=f(wx+b)\]
...
\begin{itemize}
	\item vengono forniti alla rete input e output desiderati;
	\item il modello calcola gli output a partire dagli input forniti; questo step prende il nome di \textit{forward pass}; 
